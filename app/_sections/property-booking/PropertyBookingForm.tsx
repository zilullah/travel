...
export const PropertyBookingForm: React.FC<PropertyBookingFormProps> = ({ property }) => {
  const { values, handleChange, handleSubmit } = usePropertyBookingForm(property);

  return (
    <div className="bg-white p-6 sm:p-8 rounded-[23px] border border-[#BAE6FD] text-[#0C4A6E] shadow-lg">
      <h3 className="text-xl font-bold mb-2 text-[#0C4A6E]">Inquire Legal Dossier & Site Tour</h3>
      <p className="text-xs text-[#486581] mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-[#0C4A6E] uppercase">
            Questions / Specific Interest
          </label>
          <textarea
            rows={3}
            placeholder="Contoh: Menanyakan struktur PT PMA, perizinan villa, atau proyeksi sewa harian..."
            value={values.notes}
