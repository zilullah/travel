import { IPackageRepository, TourPackageFilter } from '../repositories/package.repository.interface';
import { TourPackage, PricingTier, PackageStatus } from '../domain/package.types';
import { validateTourPackage, generateSlug } from '../domain/package.validation';
...
export class PackageService {
  ...
  async createPackage(data: any): Promise<TourPackage> {
    ...
    return this.packageRepo.create(validData as any);
  }

  async updatePackage(id: string, data: Partial<TourPackage>): Promise<TourPackage> {
    if (!id) throw new Error('Package ID is required');
  ...
  async updatePackageStatus(id: string, status: PackageStatus): Promise<TourPackage> {
  ...
  async updatePricingTiers(packageId: string, tiers: PricingTier[]): Promise<PricingTier[]> {
    if (!packageId) throw new Error('Package ID is required');

    // Invariant checks
    for (const tier of tiers) {
      if (tier.minPax > tier.maxPax) {
        throw new Error(`Tier ${tier.tierName}: minPax (${tier.minPax}) cannot be greater than maxPax (${tier.maxPax})`);
      }
